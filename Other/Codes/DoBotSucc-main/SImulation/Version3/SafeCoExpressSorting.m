    ...
    methods
        ...
        function start = SafeCoExpressSorting(initpos)
            ...
            if nargin == 0; %% returns the number of function input arguments given in the call to the currently executing function
                ...
                initx = 0.14; %% Initial x variable for UR5 position
                inity = -1; %% Initial y variable for UR5 position
...
%             end
        ...
        %for counter = 0:1:3
            ...
            while i <= 2 %array of Brick Transforms
                start.rb_trans(:,:,i) = transl(-1,0,-0.4);
                start.gb_trans(:,:,i) = transl(-1,0,-0.4);
                start.bb_trans(:,:,i) = transl(-1,0,-0.4);
...
%         isCollision = 0;
        ...
        function Main(start)
            ...
            start.robot.model.base = start.DoBotSpawn;
            ...
            redBrickStatus = 1; %1 = on convayer, 2 = picked up, 3 = put down
            start.rb_trans(:,:,1) = transl(-0.12,-1.8,0.0)
            while start.running == true
                
               
               if redBrickStatus == 1
                j=1;
                    start.rb_trans(:,:,j) = start.rb_trans(:,:,j)*transl(0,0.00091,0);
                    %start.gb_trans(:,:,j) =
                    %transl(-0.65+j*0.25,-1.5+(i*0.003),0)*trotz(0.02*i); -1.8+(i*0.00091)
                    %start.bb_trans(:,:,j) = transl(-0.65+j*0.25,-2+(i*0.003),0)*trotz(0.02*i);
               end
...
%                 start.DoBot.fkine(DoBotq_i); %% For Camera sensor.. unsure if needed here
          ...
          %------------test controller in environment
                 
    velread = start.controller.readVel();
    if norm(velread) ==0
        q = start.controller.readq();
        qrad = deg2rad(q);
        qrad(4) = -qrad(3)-qrad(2);% make link 4 || to ground     
        robotq = qrad;
        
    else % rmrc section of if statement
        endv = start.controller.readVel();
        endvel = [endv(1) ; endv(2) ; endv(3);0; 0;endv(4)];
        endvel = endvel/300 % scaling down endvel
        jac = start.robot.model.jacob0(start.robot.model.getpos);
        qadd = pinv(jac)*endvel;
        robotq = robotq+qadd'; 
        
    end % end of if velread
    
    hold on
    start.robot.model.animate(robotq);
    disp(start.robot.model.fkine(start.robot.model.getpos));
          %--------------end test cnt in en        
